...
public final class VerifiableIntArray<X extends Throwable> extends AbstractVerifiableObject<int[], X> {
    ...
    /**
     * Create a new {@link VerifiableIntArray} instance with the given values.
     *
     ...
     */
    public VerifiableIntArray(FailedValidationExecutor<X> failedValidationExecutor, int[] subject, String message) {
     ...
     */
    public int[] hasLengthOf(int n) throws X {
        int[] subject = getSubject();
     ...
     */
    public int[] hasLengthOfAtLeast(int n) throws X {
        int[] subject = getSubject();
        if (null == subject || subject.length < n) {
            fail("t -> t.length >= " + ValidityUtils.describe(n));
        }
     ...
     */
    public int[] hasLengthOfAtMost(int n) throws X {
        int[] subject = getSubject();
        if (null == subject || subject.length > n) {
            fail("t -> t.length <= " + ValidityUtils.describe(n));
        }
